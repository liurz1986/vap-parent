...
public class FileFilterUtil {
    ...
    public static List<String> fileTypes = Arrays.asList("zip","rar","xls","xlsx","csv","txt","vapbak","xml",
            "log","data","doc","docx","gz","png","jpg","bmp","gif","ppt","pptx","tif","pjp","jfif","ico",
            "tiff","svg","xbm","jxl","webp","pjpeg","avif","jpeg","pdf","html");

    private static final long F_SIZE = 500*1024*1024;
    ...
    public static String getMultiFileType(MultipartFile file) {
        String res = null;
        ...
        try {
            ...
            while (keyIter.hasNext()) {
                ...
                if (key.toLowerCase(Locale.ENGLISH).startsWith(fileCode.toLowerCase(Locale.ENGLISH)) || fileCode.toLowerCase(Locale.ENGLISH).startsWith(key.toLowerCase(Locale.ENGLISH))) {
    ...
    public static boolean validFileType(String suffix) {
        return fileTypes.contains(suffix.toLowerCase(Locale.ENGLISH));
    }

    public static boolean checkFileSize(MultipartFile file) {
       return !(file.getSize()<=F_SIZE);
    }

    public static boolean checkFileExt(String suffix) {
        return !fileTypes.contains(suffix.toLowerCase(Locale.ENGLISH));
    }

    public static boolean checkFileHeaderType(MultipartFile file) {
        String fileType = getMultiFileType(file);
