...
public class Apriori {
    ...
    public Map<List<String>,Integer> getFC(){
        ...
        while (itemkFcMap!=null && itemkFcMap.size()!=0){
            ...
            //对候选集项进行累加计数
            Map<List<String>, Integer> finalCandidateCollection = candidateCollection;
            candidateCollection.keySet().stream().forEach(candidate ->{
                for (String trans : transList){
                    ...
                    if (flag){
                        finalCandidateCollection.put(candidate, finalCandidateCollection.get(candidate)+1);
                    }
                }
            });
    ...
    private Map<List<String>,Integer> getCandidateCollection(Map<List<String>,Integer> itemkFcMap){
        ...
        //连接
        itemkSet1.stream().forEach(itemk1 ->{
