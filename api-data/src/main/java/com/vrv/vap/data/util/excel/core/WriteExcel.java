...
//                System.out.println(nAbsCol);
//                System.out.println(nChars*256);
        ...
        public int getAbsRow() {
            return getRowCount() + getHdrCount() + getBaseRow();
        }
...
//        System.out.println(sMsg);
     ...
     */
    private static Exception e(String s) {
     ...
     * @param sText  The comment text.
     *               Use a "\r\n" to cause a line break.
     * @param bFixed The text Font will be fixed if true, otherwise the default proportional font.
     * @return WriteExcel for chaining purposes.
     */
     ...
     * @return WriteExcel for chaining purposes.
     * @see <a href={@docRoot}/overview-summary.html#generic-types>Standard Types</a>.
     * @see <a href={@docRoot}/overview-summary.html#style-choice>Choosing a Cell Style</a>.
     */
     ...
     * opened with {@link com.vrv.vap.util.excel.core.ReadExcel ReadExcel}.
     * <p>
     ...
     * @see <a href={@docRoot}/overview-summary.html#builtin-names>Builtin Style Names</a>.
     * @see <a href={@docRoot}/overview-summary.html#style-choice>Choosing a Cell Style</a>.
     */
    ...
    public void closeByStream(OutputStream outputStream) throws IOException {
     ...
     *
     * @param sLinkSty  Style of index to target link.
     ...
     * @param nRow      Link row on source page.
     * @param nCol      Link column on source page.
     ...
     * @param nIdxRow      Link row on index page.
     * @param nIdxCol      Link column on index page.
     ...
     *                     <p>
     *                     Other style values may be used provided they have been added with the {@link WriteExcel#addStyleDefn addStyleDefn} method
     * @param nIdxLnkRow   Link row on target page.
     * @param nIdxLnkCol   Link column on target page.
     */
     ...
     * @param nTargRow     Link row on target page.
     * @param nTargCol     Link column on target page.
     ...
     * @param nSrcRow      Link row on source page.
     * @param nSrcCol      Link column on source page.
     */
     ...
     * @param bKeepStyle The existing style format is preserved if true. Otherwise a new format is determined according to the parsing of <code>sVal</code>.
     * @return WriteExcel for chaining purposes.
     */
        ...
        if (sDataFmt != null) {
            ...
            if ((!sDataFmt.contains(";")) && (bShowNegAsRed || (sNegFmt != null))) {
                if (sNegFmt != null) {
                    switch (sNegFmt) {
                        case "":
                            oDF.sDataFmt += ";" + (bShowNegAsRed ? "[Red]" : "") + oDF.sDataFmt;
                            break; // no negative indicator
                        case "-":
                            oDF.sDataFmt += ";" + (bShowNegAsRed ? "[Red]" : "") + "-" + oDF.sDataFmt;
                            break; //- sign
                        case "()":
                            oDF.sDataFmt += ";" + (bShowNegAsRed ? "[Red]" : "") + "(" + oDF.sDataFmt + ")";
                            break; // (Accounting style)
                        default:
                            throw e("Negative format of " + sNegFmt + " not implemented");
                    }
                }
            }
        }
    ...
    private CellStyle performStyleMerge(StyleDef oSD, StyleDef oRowSD, DataFmt oDF) throws Exception {
        ...
        if (oSD != null) {
            ...
            switch (oSA.cHorzAlign) {
                case 'L':
                    oCS.setAlignment(HorizontalAlignment.LEFT);
                    break;
                case 'C':
                    oCS.setAlignment(HorizontalAlignment.CENTER);
                    break;
                case 'R':
                    oCS.setAlignment(HorizontalAlignment.RIGHT);
                    break;
            }
            switch (oSA.cHorzAlign) {
                case 'T':
                    oCS.setVerticalAlignment(VerticalAlignment.TOP);
                    break;
                case 'M':
                    oCS.setVerticalAlignment(VerticalAlignment.CENTER);
                    break;
                case 'B':
                    oCS.setVerticalAlignment(VerticalAlignment.BOTTOM);
                    break;
            }
            switch (oSA.cTot) {
                case '-':
                    oCS.setBorderTop(BorderStyle.MEDIUM);
                    break;
                case '~':
                    oCS.setBorderTop(BorderStyle.DOUBLE);
                    break;
                case '=':
                    oCS.setBorderTop(BorderStyle.DOUBLE);
                    oCS.setBorderBottom(BorderStyle.DOUBLE);
                    break;
            }
    ...
    private StyleAttrs parseStyleAttrs(StyleDef oSD) throws Exception {
        ...
        if ((s = applyMatcher(sStyStr, oM2FG)) != null) {
            oSA.bNeedFont = true;
            setColorRGB(true, oSA, s);
        }
        if ((s = applyMatcher(sStyStr, oM2BG)) != null) {
            setColorRGB(false, oSA, s);
        }
        if ((s = applyMatcher(sStyStr, oM2CE)) != null) {
            oSA.sCustExit = s.substring(3, s.length() - 1);
        }

        if ((s = applyMatcher(sStyStr, oM2FF)) != null) {
            oSA.bNeedFont = true;
            oSA.sFontFamily = s;
        }
        if ((s = applyMatcher(sStyStr, oM2f)) != null) {
            oSA.bNeedFont = true;
            oSA.sFontFamily = sFntFix;
        }
        if ((s = applyMatcher(sStyStr, oM2b)) != null) {
            oSA.bNeedFont = true;
            oSA.bBold = true;
        }
        if ((s = applyMatcher(sStyStr, oM2i)) != null) {
            oSA.bNeedFont = true;
            oSA.bItalic = true;
        }
        if ((s = applyMatcher(sStyStr, oM2s)) != null) {
            oSA.bNeedFont = true;
            oSA.bStrike = true;
        }
        if ((s = applyMatcher(sStyStr, oM2l)) != null) {
            oSA.bNeedFont = true;
            oSA.bLink = true;
        }
        if ((s = applyMatcher(sStyStr, oM2pt)) != null) {
            oSA.bNeedFont = true;
            oSA.dPoints = Double.parseDouble(s);
        }

        if ((s = applyMatcher(sStyStr, oM2L)) != null) {
            oSA.cHorzAlign = s.charAt(0);
        }
        if ((s = applyMatcher(sStyStr, oM2C)) != null) {
            oSA.cHorzAlign = s.charAt(0);
        }
        if ((s = applyMatcher(sStyStr, oM2R)) != null) {
            oSA.cHorzAlign = s.charAt(0);
        }
        if ((s = applyMatcher(sStyStr, oM2T)) != null) {
            oSA.cVertAlign = s.charAt(0);
        }
        if ((s = applyMatcher(sStyStr, oM2M)) != null) {
            oSA.cVertAlign = s.charAt(0);
        }
        if ((s = applyMatcher(sStyStr, oM2B)) != null) {
            oSA.cVertAlign = s.charAt(0);
        ...
        if ((s = applyMatcher(sStyStr, oM2tot)) != null) {
            oSA.bNeedFont = true;
            oSA.cTot = s.charAt(0);
        }
        if ((s = applyMatcher(sStyStr, oM2sub)) != null) {
            oSA.bNeedFont = true;
            oSA.cTot = s.charAt(0);
        }
        if ((s = applyMatcher(sStyStr, oM2fin)) != null) {
            oSA.bNeedFont = true;
            oSA.cTot = s.charAt(0);
        }
        ...
        for (DataFmt oDF : oDataFmts) {
            if (oDF.oM == null) {// catch all
                ...
                if ((oSF.sData != null) && (oDF.oM.reset(oSF.sData).find())) {
                    ...
                    sPureStr = sPureStr.replaceAll("^(0+)", "");
                    sPureStr = sPureStr.length() == 0 ? "0" : sPureStr;
                    oC.setCellValue(sPureStr);
                    //log("Insert "+oSF.sData+" "+sData+" "+sPureStr+" as "+oDF.sDataFmt+" "+oDF.oM);
//                    if (sPureStr.length() >= 15) {
//                        oC.setCellValue(sPureStr);
//                    } else {
//                        if (oDF.bInteger) {
    ...
    private String getCellAsStr(Cell oC) throws Exception {
        ...
        switch (oType) {
            ...
            case FORMULA:
                ...
                return s;
            default:
                throw e("Bad Cell type:" + oType + " Col=" + oC.getColumnIndex());
        }
    }
        ...
        for (int i = 0, iMax = oA.oHdrs.size(); i < iMax; i++) {
            ...
            for (HdrCol oHC : oHCs) {
                ...
                if (bMerge) {
                    ...
                    if (bLast) {
                        int colWidth = oHC.nMaxStr * oHC.nWidthMult;
                        if (colWidth < 255*256) {
                            oS.setColumnWidth(nCol, colWidth < 3000 ? 3000 : colWidth);
                        } else {
                            oS.setColumnWidth(nCol, 6000);
                        }
                    }
                }
            }
