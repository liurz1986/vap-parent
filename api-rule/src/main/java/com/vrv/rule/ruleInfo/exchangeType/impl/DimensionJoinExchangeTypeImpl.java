 ...
 *
 */
public class DimensionJoinExchangeTypeImpl implements ExchangeType {

	private static Logger logger = LoggerFactory.getLogger(DimensionJoinExchangeTypeImpl.class);
	...
	public DataStreamSourceVO exchangeType(ExchangeVO exchangeVO) {
		...
		Attach attatch = ExchangeUtil.getDimensionAttatchInfos(filterConfigObject, exchanges.getSources(),"dimension");
		
		DimensionConfig dimensionConfig = ExchangeUtil.getDimensionAttachOptions(attatch);
		...
		JoinDimensionTable joinDimensionTable = new JoinDimensionTable(inputFieldInfoVOs, outputFieldInfoVOs, dimensionConfig,filterCode,ruleCode);
		DataStream<Row> unorderedWait = AsyncDataStream.unorderedWait(dataStreamSource, joinDimensionTable, 5, TimeUnit.SECONDS, 1000).name(filterOperator.getDesc());
