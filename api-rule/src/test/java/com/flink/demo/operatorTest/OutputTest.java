...
import com.vrv.rule.model.AlarmObj;
import com.vrv.rule.model.Configs;
...
public class OutputTest {
	...
	public void initRuleOperator(){
	   ...
	   filterOpertorVO.setName("agg+eventTime");
	   filterOpertorVO.setSourceIds(sourcesIds);
	   filterOpertorVO.setVersion("1");
	   Outputs outputs = new Outputs();
	   outputs.setId(UUID.randomUUID().toString());
	   outputs.setType("kafka");
	   Configs config = new Configs();
	   config.setName("kafka-test-19");
	   outputs.setConfig(config);
	   
	   Outputs output1s = new Outputs();
	   output1s.setId(UUID.randomUUID().toString());
	   output1s.setType("alarmdeal");
	   Configs config1 = new Configs();
	   config1.setName("flink-wiki-demo");
	   AlarmObj alarmObj = new AlarmObj();
	   alarmObj.setSrcIp("device_ip");
	   alarmObj.setDstIp("device_ip");
	   config1.setAlarmObj(alarmObj);
	   output1s.setConfig(config1);
	   
	   
	   List<Outputs> list = new ArrayList<>();
	   list.add(outputs);
	   list.add(output1s);
	   filterOpertorVO.setOutputs(list);
	   filterOpertorVO.setOperatorType("analysis");
	}
...
//		logicOperator2.setFilters(logicList2);
	...
	@Test
	public void testFilterConfigObject1(){
		Gson gson = new Gson();
		AggregateOperator aggregateOperator = new AggregateOperator();
		aggregateOperator.setOperator("count");
		//aggregateOperator.setField("report_ip_num");
		String json = gson.toJson(aggregateOperator);
		System.out.println(json);
	}
