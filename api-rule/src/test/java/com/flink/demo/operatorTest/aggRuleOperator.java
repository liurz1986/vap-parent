...
public class aggRuleOperator {
	...
	public void initRuleOperator(){
	   ...
	   filterOpertorVO.setName("agg+filter+window+eventTime");
...
//		logicOperator2.setFilters(logicList2);
	...
	public void tesRows() {
		...
		Row row1 = Row.of("1","2","3");
		Row row2 = Row.of("4","5","6");
		Row row3 = Row.of("3","4");
		list.add(row1);
		list.add(row2);
		Row lastRow = Row.of("1","2","3",row3,list);
		List<Row> rowArr = new ArrayList<>();
		int arity = lastRow.getArity();
		Row newRow = new Row(8);
		for (int i = 0; i < arity; i++) {
			Object field = lastRow.getField(i);
			if(field instanceof List<?>) {
				List<Row> rows = (List<Row>)field;
				int tmp=i;
				for (int j = 0; j < rows.size(); j++) {
					Row rownow = Row.copy(newRow);
					Row row = rows.get(j);
					for (int k = 0; k < row.getArity(); k++) {
						Object field2 = row.getField(k);
						rownow.setField(i,field2);
						i++;
					}
					i=tmp;
					rowArr.add(rownow);
				}
			}else if(field instanceof Row) {
				int tmp=i;
				Row row = (Row)field;
				Row rownow = Row.copy(newRow);
				for (int k = 0; k < row.getArity(); k++,tmp++) {
					Object field2 = row.getField(k);
					rownow.setField(tmp,field2);
				}
				newRow=rownow;
			}else{
				newRow.setField(i, lastRow.getField(i));
			}
		}
	...
	@Test
	public void tesRows1() {
		List<Row> list = new ArrayList<>();
		Row row1 = Row.of("1","2","3");
		Row row2 = Row.of("4","5","6");
		list.add(row1);
		list.add(row2);
		Row lastRow = Row.of("1","2","3",row1,list);
