...
public class DateUtil {
	...
	public static final String SLASH_DATE = "yyyy/M/d";
	...
	private static long getByField(Calendar beginCalendar, Calendar endCalendar, int calendarField) {
		long endField = endCalendar.get(calendarField);
		long beginField = beginCalendar.get(calendarField);
		return endField - beginField;
	}

	public static long getTime(Date beginDate, Date endDate) {
	...
	public static int getMonthByDateFormat(String dateStr,String dateFormat) throws ParseException {
	 ...
	 */
	@SuppressWarnings("deprecation")
	public static int getYearByDateFormat(String dateStr,String dateFormat) throws ParseException {
		 SimpleDateFormat simpleDateFormat = new SimpleDateFormat(dateFormat);
		 Date date = simpleDateFormat.parse(dateStr);
		 Calendar c = Calendar.getInstance();  
		 c.setTime(date);
	 ...
	 * @return
	 */
	public static long getTimeTostamp(String deadLineStr,String datePattern) {
		long time = 0;
		String strDateTime = String.valueOf(deadLineStr);
		SimpleDateFormat formatTool = new SimpleDateFormat();
		formatTool.applyPattern(datePattern);
		try {
			Date date = formatTool.parse(strDateTime);
			time = date.getTime();
		} catch (ParseException e) {
			log.error("时间解析时间戳出现错误", e);
		}
		return time;
	}
