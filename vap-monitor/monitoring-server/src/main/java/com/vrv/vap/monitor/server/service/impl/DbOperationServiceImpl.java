...
import com.vrv.vap.monitor.server.common.config.DbBackupConfig;
...
import com.vrv.vap.monitor.server.service.DbOperationService;
...
public class DbOperationServiceImpl extends BaseServiceImpl<DbOperationInfo> implements DbOperationService {
    ...
    public Result backup(DbOperationInfo dbOperationInfo) {
        ...
        dbOperation.setUuid(UUID.randomUUID().toString());
        ...
        try {
            ...
            BackupUtil.createBakFile(filePath, foldName, "sys", JsonUtil.objToJson(sysInfoMap));
    ...
    public Result restart(DbOperationInfo dbOperationInfo) {
        ...
        return new Result("999","系统繁忙，请稍后再试！");
    }
    ...
    public DbOperationInfo uploadFile(MultipartFile file) {
        ...
        Date date = new Date();
        dbOperationInfo.setUuid(UUID.randomUUID().toString());
        ...
        dbOperationInfo.setDataTypes(fileType.replace("&", ","));
    ...
    public Result downloadFile(String uuid, HttpServletResponse response) {
        ...
        operationInfo.setUuid(UUID.randomUUID().toString());
